...
import { Button, NumberInput, 密码Input, Select, Slider, Switch, Tabs, Text, TextInput, Textarea } from "@mantine/core";
import { Option } from "../../core/options/option";
import 设置Option from "./option";
import { select设置Option } from "../../store/settings-ui";
...
export function PluginOptionWidget(props: { pluginID: string, option: Option, chatID?: string | null | undefined, context: Context }) {
    const requestedOption = useAppSelector(select设置Option);
    ...
    switch (renderProps.type) {
        ...
        case "password":
            component = (
                <密码Input label={!option.displayAsSeparateSection ? renderProps.label : null}
                    placeholder={renderProps.placeholder}
                    disabled={renderProps.disabled}
                    value={value}
                    onChange={e => setValue(e.target.value)} />
    ...
    const elem = <OptionWrapper class名称={(focused && !option.displayAsSeparateSection) ? 'focused' : ''}>
        {component}
    ...
    if (option.displayAsSeparateSection) {
        return <设置Option heading={renderProps.label} focused={focused}>
            {elem}
...
export default function 设置Tab(props: {
    name: string;
    children?: any;
}) {
    ...
    const optionSets = [...globalOptions, ...pluginMetadata]
        .map(metadata => ({
            ...
            options: metadata.options.filter(o => o.displayOn设置Screen === props.name),
            resettable: metadata.options.filter(o => o.displayOn设置Screen === props.name && o.resettable && !o.displayAsSeparateSection).length > 0,
            collapsed: metadata.options.filter(o => o.displayOn设置Screen === props.name && o.displayAsSeparateSection).length > 0,
        ...
        <Tabs.Panel value={props.name}>
            <设置>
                ...
                {optionSets.map(({ name, id, description, options, resettable, collapsed }) => <>
                    <设置Option heading={name} description={description} collapsed={collapsed} key={id}>
